...
- What is put to a person for a decision is in the message that asks. A sentence for a document, a requirement's text, a step of a plan: the message carries the text as it would be written, never "as shown above" — a pointer to an earlier message is a key handed over instead of the thing, the failure a bare identifier is. Stated in both guides and in the `srs` and `srs-new` procedures (`FR-SKILL-320`).
- A number widens, and nothing is renamed. An identifier's number has three digits or more, written without a leading zero beyond the third — after `990` comes `1000` — in every grammar the framework reads: a requirement's, a register record's, an element's; and every tool orders identifiers by their number, so `1000` follows `990` and not `100`. What was written stays as it is (`INV-SPEC-080`, `INV-SPEC-090`, ADR-0028).
- An area is a file or a directory of files, by the thousand. `10-fr-<area>.md` holds the first thousand; the first number past it moves the file whole to `10-fr-<area>/000-999.md` and opens `1000-1999.md`, and the checker reads both forms alike, a `README.md` inside the directory left to people. A number outside the range its file's name states is a warning named `file-range`, with the move in the message; the `srs-new` procedure makes the move at the one moment it happens (`FR-CHK-250`, `FR-CHK-260`).
- A path an element carries and nobody has is reported. The architecture checker warns, under `carrier-missing`, of a `carries` entry that is not in the repository, naming the element and the path — and, where git can tell, the path the file was renamed to and the commit, or the commit that deleted it; a project may turn the rule off like any other. The record stays the project's to fix: the layer writes the map and nothing else (`FR-ARCH-280`).
- A class III verdict is the reader's. The grounds checker no longer compels a verdict from how far a small sample can miss where the hypothesis is class III — a reading of two people is a reading, and the interval at that size compelled `supported` whatever both said; the threshold stays required and still binds by its own words, and the reader stays named (`FR-GND-550`).
...
- Adoption installs the standard. A project that adopts the framework with a `specs/README.md` of its own has that file set aside as `specs/archive/README-before-srs-dd.md`, byte for byte, and the standard installed in its place with the marker; the installer names both paths, lists the move under `--dry-run`, and stops before writing anything when the archive path is already taken. The checker enforced the standard's rules from the first run either way, and the document that claimed to win was the one that had no say; the `srs-init` procedure now sorts what the former document said beyond the standard — a rule of the project's own to the constitution, a description of the system to the introduction and the glossary (`FR-INIT-230`, ADR-0029). `FR-INIT-040` narrows to the files that carry requirements, which adoption still never touches.
...
- A project adopted before this release still holds its own `specs/README.md`, and an upgrade does not replace a file without the marker. To bring it to the same state a new adoption reaches: `git mv specs/README.md specs/archive/README-before-srs-dd.md`, then `python3 <framework-clone>/tools/srs_init.py <target> --defaults` installs the standard in its place; the `srs-init` procedure's adopt step says what to do with the archived file.
